...
export default {
    async fetch(request, env) {
        ...
        // Multiplayer WebSocket entry point. main.js connects here as
        // `${wsProtocol}//${location.host}/ws?game_id=...`. Every player
        // with the same game_id lands in the same Durable Object instance
        // (idFromName makes that mapping deterministic), so the room can
        // broadcast positions/chat between them.
        //
        // Identity comes from the session cookie, not a client-supplied
        // id/username -- same trust boundary as every other authenticated
        // route in this file. The WebSocket upgrade request still carries
        // cookies, so this works the same way any other fetch would.
        if (url.pathname === "/ws") {
            ...
            doUrl.searchParams.set("user_id", String(session.id));
...
//   client -> server: { type: 'partSync', name, props }
//   client -> server: { type: 'dynamicSync', parts: [{ name, x, y, z, rx, ry, rz }] }
//   client -> server: { type: 'claim', name, userId }
//   server -> client: { type: 'state', players: { [userId]: { pos, rot, anim } } }
//   server -> client: { type: 'chat', username, text }
//   server -> client: { type: 'partSync', name, props }
//   server -> client: { type: 'dynamicSync', parts: [...] }
//   server -> client: { type: 'claim', name, userId }
//   server -> client: { type: 'partsSnapshot', parts: { [name]: props }, owners: { [name]: userId } }
export class GameRoom {
    ...
    async webSocketMessage(ws, message) {
        ...
        if (msg.type === "move") {
            ws.serializeAttachment({
                ...info,
                pos: Array.isArray(msg.pos) ? msg.pos : info.pos,
                rot: Array.isArray(msg.rot) ? msg.rot : info.rot,
                anim: typeof msg.anim === "string" ? msg.anim : info.anim
            });
            this.broadcastState();
    ...
    broadcastState() {
        ...
        for (const socket of this.state.getWebSockets()) {
            ...
            if (info) {
                players[info.id] = { pos: info.pos, rot: info.rot, anim: info.anim };
            }
        }
